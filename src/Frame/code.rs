...
pub fn main() {
    let pic_with_frame: gtk::Picture = workbench::builder().object("with_frame").unwrap();
    let pic_without_frame: gtk::Picture = workbench::builder().object("without_frame").unwrap();

    let textview_with_frame: gtk::TextView =
        workbench::builder().object("textview_with_frame").unwrap();
    let textview_without_frame: gtk::TextView = workbench::builder()
        .object("textview_without_frame")
        .unwrap();

    let file = gtk::gio::File::for_uri(&workbench::resolve("./image.png"));
    ...
    textview_with_frame.set_buffer(Some(&buffer));
    textview_without_frame.set_buffer(Some(&buffer));

    pic_with_frame.set_file(Some(&file));
    pic_without_frame.set_file(Some(&file));
}
