...
pub fn main() {
    ...
    // Make clamp available in the closure using glib::clone
    // https://gtk-rs.org/gtk-rs-core/stable/latest/docs/glib/macro.clone.html
    let increase = glib::clone!(@weak clamp => move |_: &gtk::Button| {
    ...
    let decrease = glib::clone!(@weak clamp => move |_: &gtk::Button| {
      let current_size = clamp.maximum_size();
      let current_threshold = clamp.tightening_threshold();
    ...
    button_increase.connect_clicked(increase);
    button_decrease.connect_clicked(decrease);
}
