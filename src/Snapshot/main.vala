...
public class Chessboard : Gtk.Widget {
    ...
    public Chessboard() {
        ...
        this.knight_stroke = new Gsk.Stroke(2.0f);
        this.knight_path.get_stroke_bounds(this.knight_stroke, out this.knight_bounds);

        // Add drag gesture and connect it to signals
        this.gesture = new Gtk.GestureDrag();
        this.add_controller(gesture);

        this.gesture.drag_begin.connect(() => {
            var knight_hitbox = Graphene.Rect();
            knight_hitbox.init(this.initial_x, this.initial_y, PIECE_SIZE, PIECE_SIZE);
            var cursor_pos = Graphene.Point();
            cursor_pos.init((float) x, (float) y);
            this.gesture_started = knight_hitbox.contains_point(cursor_pos);
        });

        this.gesture.drag_end.connect((offset_x, offset_y) => {
            if (!this.gesture_started)return;
            this.initial_x += (float) offset_x;
            this.initial_y += (float) offset_y;
            this.queue_draw();
            this.gesture_started = false;
        });
        this.gesture.drag_update.connect((offset_x, offset_y) => {
            if (!this.gesture_started)return;
            this.x = this.initial_x + (float) offset_x;
            this.y = this.initial_y + (float) offset_y;
            this.queue_draw();
        });
    }
    ...
    public override void snapshot(Gtk.Snapshot snapshot) {
        ...
        anchor.init(this.x, this.y);
        float factor = (float) PIECE_SIZE / this.knight_bounds.get_width();
        var transformation = new Gsk.Transform().translate(anchor).scale(factor, factor);
        snapshot.transform(transformation);
        snapshot.append_stroke(this.knight_path, this.knight_stroke, this.knight_color);
    }
